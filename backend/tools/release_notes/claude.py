...
def _format_issue(i: dict) -> str:
    lines = [f"[{i['key']}] {i['type']} | {i['priority']} | {i['status']}",
             f"Summary: {i['summary']}"]
...
def regenerate_section(config: dict, version_info: dict, issues: list,
                       section: str, current_text: str, instruction: str) -> str:
    size_key = DEFAULTS.get(section, "small")
    instruction_block = f"Additional instructions: {instruction}\n\n" if instruction.strip() else ""
...
def generate_with_ai(config: dict, version_info: dict, issues: list, sizes: dict) -> dict:
    ...
    def extract(t, start, end=None):
        ...
        for line in lines:
            if start in line:  active = True; continue
            if end and end in line: break
            if active: result.append(line)
        return "\n".join(result).strip()

    return {
        "prompt":                prompt,
        "shortDescription":      extract(text, "<<<SHORT_DESCRIPTION>>>",     "<<<FULL_DESCRIPTION>>>"),
        "fullDescription":       extract(text, "<<<FULL_DESCRIPTION>>>",      "<<<BUSINESS_JUSTIFICATION>>>"),
