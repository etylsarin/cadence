...
STORY_SUMMARIES = [
    "Add bulk export to {area}",
    "Redesign {area} filters",
    "Support multi-currency in {area}",
    "Migrate {area} to new API gateway",
    "Inline editing for {area} table",
    "Add audit log to {area}",
    "Improve {area} search relevance",
    "Lazy-load {area} dashboard widgets",
    "New onboarding flow for {area}",
    "Consolidate {area} notification settings",
    "Add CSV import validation to {area}",
    "Role-based access for {area} admin",
    "Pagination for {area} history view",
    "Dark mode for {area} screens",
    "Localize {area} into DE and FR",
    "Configurable SLA thresholds in {area}",
]
BUG_SUMMARIES = [
    "{area} export drops rows with unicode names",
    "500 when saving empty {area} form",
    "{area} totals off by one day across DST",
    "Duplicate notifications from {area} watcher",
    "{area} chart axis overlaps on small screens",
    "Session expires mid-{area} wizard",
    "{area} search ignores diacritics",
    "Stale cache after {area} bulk update",
    "{area} PDF render cuts long tables",
    "Race condition in {area} autosave",
]
TASK_SUMMARIES = [
    "Update {area} dependencies",
    "Document {area} runbook",
    "Add monitoring to {area} jobs",
    "Cleanup deprecated {area} flags",
    "Tune {area} DB indexes",
    "Rotate {area} service credentials",
]
SPIKE_SUMMARIES = [
    "Evaluate vendor options for {area}",
    "Prototype {area} caching layer",
    "Investigate {area} latency spikes",
]
AREAS = [
    "billing",
    "checkout",
...
STORY_FLOW = [
    ...
    ("In Progress", 1, 8),
    ("Code Review", 0.2, 2.5),
    ("Ready For Testing", 0.1, 3),
    ("In Testing", 0.5, 4),
    ("Ready for UAT", 0.1, 2),
    ("UAT in Progress", 0.5, 3),
...
def build_flow(flow, speed: float, rework: bool):
    ...
    if rework and len(flow) > 5:  # send it back from testing once
        ...
        if i:
            out[i + 1 : i + 1] = [
                ("In Progress", rng.uniform(0.5, 3) * speed),
        ...
        histories.append(
            {
                "created": iso(t),
                "items": [
                    {"field": "status", "fromString": status, "toString": to_status}
                ],
            }
        )
    return histories, t
...
def base_fields(
    ...
    f = {
        ...
        "assignee": {"displayName": rng.choice(ASSIGNEES[project])}
        if rng.random() < 0.9
        else None,
        "created": iso(created),
        "updated": iso(updated),
        ...
        "customfield_10008": f"{project}-{rng.randint(90, 96) * 100}"
        if rng.random() < 0.7
        else None,
    }
...
def add_bug_fields(fields: dict, escaped: bool):
    ...
    if escaped:
        ...
        fields["customfield_12589"] = [
            {"value": rng.choice(["UAT [UAT]", "SIT [SIT]", "Dev [DEV]"])}
        ]
        fields["customfield_12017"] = {
            "value": rng.choice(["UAT", "SIT", "Development", "Internal Testing"])
        }
...
def completed_issue(project: str, itype: str, terminal_dt: datetime):
    ...
    steps = build_flow(
        flow, profile["speed"], rework=(itype == "Story" and rng.random() < 0.2)
    )
    ...
    pool = {
        ...
        "Bug": BUG_SUMMARIES,
        "Task": TASK_SUMMARIES,
        "Spike": SPIKE_SUMMARIES,
    }[itype]
    fields = base_fields(
        project, itype, pick_summary(pool, project), terminal, created, terminal_dt
    )
...
def inflight_issue(project: str, status: str, days_stale: int):
    ...
    pool = {"Story": STORY_SUMMARIES, "Bug": BUG_SUMMARIES, "Task": TASK_SUMMARIES}[
        itype
    ]
    fields = base_fields(
        project, itype, pick_summary(pool, project), status, created, last_transition
    )
...
def backlog_issue(project: str, epic_key: str):
    ...
    fields = base_fields(
        project, itype, pick_summary(pool, project), status, created, created
    )
    ...
    if status != "Open":
        histories = [
            {
                "created": iso(created + timedelta(days=rng.uniform(0.5, 5))),
                "items": [
...
def link_entry(link_type, direction: str, other_key: str, other_status: str):
    ...
    entry["outwardIssue" if direction == "outward" else "inwardIssue"] = {
        "key": other_key,
        "fields": {"status": {"name": other_status}},
    }
    return entry
...
def generate() -> list:
    ...
    for project in PROFILES:
        ...
        for _ in range(rng.randint(8, 12)):
            ...
            days = rng.choices(
                [rng.randint(5, 13), rng.randint(14, 45), rng.randint(46, 120)],
                weights=[2, 5, 3],
            )[0]
    ...
    for project, group in stale_by_project.items():
        ...
        add_external_link(
            group[0], LINK_TYPES[0], ext, rng.choice(["In Progress", "Open"])
        )
    ...
    if (
        len(PROJECTS) >= 2
        and stale_by_project[PROJECTS[0]]
        and stale_by_project[PROJECTS[1]]
    ):
        add_link(
            stale_by_project[PROJECTS[1]][0],
            stale_by_project[PROJECTS[0]][0],
            LINK_TYPES[0],
        )
    ...
    for project in PROFILES:
        for _ in range(rng.randint(5, 10)):
            issues.append(
                inflight_issue(project, "Approved for PROD env", rng.randint(1, 45))
            )
...
def main():
    ...
    result = subprocess.run(
        [sys.executable, "_silver2gold.py"], cwd=ROOT / "tools" / "sync"
    )
